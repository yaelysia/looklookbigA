...
def _event_metadata(event, fetched_at):
    ...
    if document.get("status") == "UNAVAILABLE":
        flags.append("DOCUMENT_FACT_EXTRACTION_UNAVAILABLE")
        if event.get("importance") == "HIGH" and quality == "PASS":
            quality = "DEGRADED"
...
def finalize_snapshot(snapshot_path):
    ...
    if isinstance(summary, dict):
        ...
        quality = _quality(status)
        flags = ["SOME_DETAIL_EVENT_SOURCES_DEGRADED"] if status == "PARTIAL" else []
        fact_enrichment = summary.get("fact_enrichment") or {}
        if fact_enrichment.get("status") == "PARTIAL":
            flags.append("FACT_ENRICHMENT_PARTIAL")
            if quality == "PASS":
                quality = "DEGRADED"
